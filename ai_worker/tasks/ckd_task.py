...
def _load():
    """predictor·stats·threshold 1회 로드(상주). 반환: (predictor, stats, threshold)."""
    global _predictor, _stats, _threshold
    ...
    return _predictor, _stats, _threshold


async def handle_ckd_job(job: CkdJob) -> None:
    """예측 후 health_checks를 갱신. 실패는 로그로 남기고 예외를 다시 올린다(호출부에서 ack)."""
    predictor, stats, threshold = await asyncio.to_thread(_load)
    ref = date.fromisoformat(job.checked_date)
    out = await asyncio.to_thread(pipeline.run_inference, job.payload, ref, predictor, threshold, stats, job.egfr)
    ...
    logger.info(
        "ckd 예측 완료 hc=%s risk=%.4f group=%s",
        job.health_check_id,
        out["ckd_risk_score"],
        out["app_group"],
    )
